...
Each domain's scores are stored alongside a content hash of the
domain definition (suites, questions, weights). When computing
scores, the system checks whether the domain definition has changed
since the last evaluation and only recomputes when necessary.
"""
...
def compute_domain_hash(domain: Domain) -> str:
    """Compute a stable hash of a domain's definition.

    The hash covers suites, questions, weights, and evaluation methods.
    If any of these change, the hash changes and scores are recomputed.
    """
    definition = {
        "id": domain.id,
        "suites": [
...
def load_domain_hashes(
    path: Path | None = None,
) -> dict[str, str]:
    """Load domain content hashes from confidence.yaml.

    Returns a dict mapping domain_id → hash. Used to detect when
    a domain's definition has changed and scores need recomputation.
    """
    if path is None:
        path = confidence_path()
    ...
    return data.get("domain_hashes", {})


def save_confidence(
    scores: dict[str, ModelConfidence],
    domain_hashes: dict[str, str] | None = None,
    path: Path | None = None,
) -> Path:
    """Save all model confidence scores to confidence.yaml.

    Args:
        scores: Model confidence scores to save.
        domain_hashes: Domain content hashes for idempotent recomputation.
    ...
    if domain_hashes:
        artifact["domain_hashes"] = domain_hashes
