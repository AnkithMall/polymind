...
class Pipeline:
    ...
    def _run_inner(
        ...
        for task in tasks:
            ...
            if a and a.model:
                ...
                if a.runtime_feasibility:
                    runtime_status = a.runtime_feasibility.status
    ...
    def _get_or_load_model(self, model_id: str) -> Llama | None:
        ...
        try:
            ...
            self._warnings.append(f"GPU load failed for {model_id} ({e}), falling back to CPU")
            try:
                llm = Llama(
                    model_path=str(model_path),
                    n_gpu_layers=0,
                ...
                self._warnings.append(f"Failed to load model {model_id}: {e2}")
                self._emit(
                    "model_load_failed",
                    Task(
                        id="load",
                        ...
                        domain="",
                        task_type=TaskType.CUSTOM,
                        metadata={"model_id": model_id, "reason": str(e2)},
                    ),
                )
                return None
